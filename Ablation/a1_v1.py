...
class SRSSB(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self.ln = norm_layer(hidden_dim)
        # 使用3x3卷积替换self_attention
        self.conv3x3 = nn.Conv2d(in_channels=hidden_dim, out_channels=hidden_dim, kernel_size=3, padding=1)
        self.drop_path = DropPath(drop_path)
        self.ffn = SMFFL(in_channels=hidden_dim)

    def forward(self, input: torch.Tensor):
        # 输入格式为[B, H, W, C]，先转为[B, C, H, W]以便进行卷积操作
        # print(input.shape)
        x = self.ln(input)
        x = x.permute(0, 3, 1, 2)  # 将[B, H, W, C] -> [B, C, H, W]
        x=self.drop_path(self.conv3x3(x))  # 应用3x3卷积
        x = x.permute(0, 2, 3, 1)  # 将[B, C, H, W] -> [B, H, W, C]
        x=x+input
        residual = x
        # block2
        x = residual + self.drop_path(self.ffn(self.ln(x)))
